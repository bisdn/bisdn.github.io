...
## Authentication events on the switch

Hostapd allows subscribing to events while running. This allows extended
logging or reacting on various authentication events, e.g. repeated failure of
authentication.

For convenience we allow putting a script at
`/etc/hostapd/wired-auth-events.sh` which will be called on each hostapd event.
To make writing such a script easier, we provide a template
`/etc/hostapd/wired-auth-events.sh.template` with most events and their passed
attributes.

Be aware that the list of events in the template file is not exhaustive, and
only contains authentication state transition events. See the [list of EAP
events](https://git.w1.fi/cgit/hostap/tree/src/common/wpa_ctrl.h#n38) for what
other events there also may be.
